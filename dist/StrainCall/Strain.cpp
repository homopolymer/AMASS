...
string Strain::get_model()
{
    string model = "";
    stringstream ss;

    // output compositional model
    for (auto iter=A.begin(); iter!=A.end(); iter++)
    {
        ss << iter->second << ":" << exp(logprob(iter->second)) << ";";
    }

    // output substitutional model
    for (auto iter1=A.begin(); iter1!=A.end(); iter1++)
    {
        for (auto iter2=A.begin(); iter2!=A.end(); iter2++)
        {
            ss << iter1->second << iter2->second << ":" << exp(logprob(iter1->second,iter2->second)) << ";";
        }
    }
